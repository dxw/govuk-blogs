...
require_once(dirname(__DIR__).'/widget_categories_dropdown_with_submit/helpers.php');
...
describe(Widget::class, function () {
    ...
    describe('->widget()', function () {
        ...
        it('outputs HTML', function () {
            ...
            expect($output)->to->equal('AAABBBArchivesCCC<label class="screen-reader-text" for="y">Archives</label><select id="y" name="archive-dropdown"><option value="">Select month</option>HELLO FROM wp_get_archives</select><button class="govuk-button" data-module="govuk-button" onclick="newLocation = document.getElementById(\'y\').value; if ( newLocation != \'\' ) { window.location = newLocation; }">Go</button>DDD');
        });
    });
});
